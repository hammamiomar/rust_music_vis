...
pub fn create_spectrogram_from_audio(
    ...
    let mut db_spectrogram = magnitude_to_db(&magnitude_spectrogram, -120.0, normalize);
...
fn convert_image_to_egui_image(img: ImageBuffer<Rgb<u8>, Vec<u8>>) -> egui::TextureHandle {
    ...
    let texture = egui::Context::new().load_texture(
        "spectrogram",
        color_image,
...
// Example usage in an egui application
...
impl SpectrogramApp {
    ...
    fn load_spectrogram(&mut self, ctx: &egui::Context, file_path: &str) {
        match create_spectrogram_from_audio(file_path, 2048, true, SpectrogramColormap::Viridis) {
            Ok(texture) => self.spectrogram = Some(texture),
            Err(err) => eprintln!("Failed to create spectrogram: {}", err),
        }
    }
}
    ...
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        egui::CentralPanel::default().show(ctx, |ui| {
            if ui.button("Load MP3").clicked() {
                // In a real app, you would use a file dialog here
                self.load_spectrogram(ctx, "path/to/your/audio.mp3");
            }
            
            if let Some(texture) = &self.spectrogram {
...
fn main() -> Result<(), eframe::Error> {
    ...
    eframe::run_native(
        ...
        options,
        Box::new(|_cc| Box::new(SpectrogramApp::new()))
    )
}